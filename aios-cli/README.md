...
Рекомендуемый способ установки через NPM (работает на Windows, Mac и Linux без необходимости установки Python):

```bash
npm install -g aios-cli
```

*Скрипт автоматически скачает оптимизированный бинарник под вашу операционную систему.*
...
> 🔒 AIOS Desktop (приложение с графическим интерфейсом на Tauri) имеет закрытый исходный код и будет распространяться отдельно через официальный сайт в виде готовых установщиков (.msi, .dmg). Этот репозиторий содержит исключительно открытый исходный код CLI-клиента и ядра (Core/Runtime/Engines).
